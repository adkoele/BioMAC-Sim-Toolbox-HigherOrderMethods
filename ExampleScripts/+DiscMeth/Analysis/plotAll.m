...
set(gca,'LineWidth', round(LineWidth/2));
if strcmp(dim,'2D')
    lgd = legend(['','BE','','','','','','', 'ME', '','','','','','', 'LIIIc-2', '','','','','','','RIIa-2',  '','','','','','','LIIIc-3', '','','','','','','RIIa-3'],...
...
%% Quantitative analysis of best trade-off method

% set counter
nBE = 1; nME = 1; nL2 = 1; nR2 = 1; nR3 = 1; nL3 = 1;

for i = 1:size(allCPU,2)
    % Log-scale the distances in the error/CPU plot
    allCPU_cur(1,i) = (log(allCPU(1,i)) - log(min(allCPU(1,:)))) / (log(max(allCPU(1,:))) - log(min(allCPU(1,:))));
    allErr_cur(1,i) = (log(allErr(1,i)) - log(min(allErr(1,:)))) / (log(max(allErr(1,:))) - log(min(allErr(1,:))));
    normDist(i) = sqrt(allCPU_cur(1,i)^2 + allErr_cur(1,i)^2);
   
    % Allocate to specific distance counter
    if allMethodID(i) == 1
        BEDist(nBE) = normDist(i);
        nBE = nBE +1;
    elseif allMethodID(i) == 2
        MEDist(nME) = normDist(i);
        nME = nME +1;
    elseif allMethodID(i) == 3
        LIIIc2Dist(nL2) = normDist(i);
        nL2 = nL2 + 1;
    elseif allMethodID(i) == 4
        RIIa2Dist(nR2) = normDist(i);
        nR2 = nR2 +1;
    elseif allMethodID(i) == 5
        LIIIc3Dist(nL3) = normDist(i);
        nL3 = nL3 +1;
    else
        RIIa3Dist(nR3) = normDist(i);
        nR3 = nR3 +1;
    end
end
...
meanME = mean(MEDist);
stdME = std(MEDist);
meanL2 = mean(LIIIc2Dist);
stdL2 = std(LIIIc2Dist);
meanR2 = mean(RIIa2Dist);
stdR2 = std(RIIa2Dist);
meanL3 = mean(LIIIc3Dist);
stdL3 = std(LIIIc3Dist);
meanR3 = mean(RIIa3Dist);
stdR3 = std(RIIa3Dist);

labels = {'BE', 'ME', 'LIIIc-2', 'RIIa-2', 'LIIIc-3', 'RIIa-3'};
colorsBox = colors(1:6,:);

means = [meanBE, meanME, meanL2, meanR2, meanL3, meanR3];
stds  = [stdBE,  stdME,  stdL2,  stdR2,  stdL3,  stdR3];
x = 1:numel(means);

%% Plot boxplots
figure; hold on
for i = 1:length(x)
    errorbar(x(i), means(i), stds(i), ...
        'o', ...
        'Color', colors(i,:), ...
...
xtickangle(30)
ylabel('Normalized mean distance to origin', ...
    'FontName', FontName, ...
    'FontSize', FontSize);

%title('Trade-off performance across discretization methods', ...
%    'FontName', FontName, ...
%    'FontSize', FontSize);
...
if size(nNodes,1) > 1
    print(gcf, fullfile([path2repo, filesep,'ParetoRes_', mov,'_' tra]), '-depsc', '-r300');
    print(gcf, fullfile([path2repo, filesep,'ParetoRes_', mov,'_' tra]), '-dpng', '-r300');
end
